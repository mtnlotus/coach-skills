...
function parseNoteDate(raw) {
    ...
    if (named) {
        ...
        if (mon)
            return `${named[3]}-${mon}-${named[2].padStart(2, "0")}`;
    }
...
export class NoteParser {
    ...
    nextNonempty(idx) {
        for (let i = idx + 1; i < this.n; i++) {
            if (this.paras[i])
                return [i, this.paras[i]];
        }
        return [-1, ""];
    }
    findIntInWindow(start, window = 10) {
    ...
    collectUntil(start, stopRe, maxLines = 20) {
        const parts = [];
        for (let i = start; i < Math.min(start + maxLines, this.n); i++) {
            const text = this.paras[i];
            if (!text)
                break;
            if (stopRe && stopRe.test(text))
                break;
            parts.push(text);
        }
        return parts.join(" ");
    ...
    parseShortTermGoals(sectionIdx, sectionEnd) {
        const goals = [];
        let current = null;
        let i = sectionIdx + 1;
        while (i < sectionEnd) {
    ...
    parsePlan(planSectionIdx) {
        ...
        for (let i = planSectionIdx + 1; i < this.n; i++) {
            ...
            if (!collecting)
                continue;
            if (/^(Arranged follow-up|Veteran agreed to follow-up)/.test(p))
                continue;
    ...
    parse() {
        ...
        if (ltEnd < 0 && ltStart >= 0)
            ltEnd = stStart > ltStart ? stStart : this.n;
        if (stEnd < 0 && stStart >= 0)
            stEnd = this.n;
