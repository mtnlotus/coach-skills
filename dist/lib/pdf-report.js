...
// ---------------------------------------------------------------------------
const mm = (n) => n * 2.835;
const VA_NAVY = [0, 63, 114];
const VA_BLUE = [0, 100, 164];
const VA_BLUE_BG = [232, 241, 250];
const VA_GREEN = [46, 133, 64];
const VA_GOLD = [180, 120, 0];
const VA_RED = [152, 29, 37];
const BAR_BG = [210, 223, 235];
const RULE_COLOR = [190, 210, 230];
const MID_GRAY = [120, 120, 120];
const DARK_TEXT = [28, 28, 28];
...
export class PHPReport {
    ...
    // Public API
    // ---------------------------------------------------------------------------
    build(php, reportDate, outputPath) {
    ...
    // ---------------------------------------------------------------------------
    _fill(c) { this.doc.fillColor(c); return this; }
    _stroke(c) { this.doc.strokeColor(c); return this; }
    _fillStroke(c) { this.doc.fillColor(c).strokeColor(c); return this; }
    // ---------------------------------------------------------------------------
    // Y-position helpers (pdfkit tracks current Y via doc.y after text)
    // ---------------------------------------------------------------------------
    getY() { return this.doc.y; }
    setY(y) { this.doc.y = y; return this; }
    ln(n) { this.doc.y += n; return this; }
    remaining() {
    ...
    // Layout primitives
    // ---------------------------------------------------------------------------
    _hRule(color = RULE_COLOR, thickness = 0.3) {
        const y = this.getY();
        this._stroke(color);
        this.doc.moveTo(L_MARGIN, y).lineTo(PAGE_W - R_MARGIN, y).lineWidth(thickness).stroke();
        this.ln(mm(2));
    }
    _sectionHeader(title, color = VA_BLUE, minSpace = mm(22)) {
    ...
    _body(text, style = "regular", size = 10, indent = 0, color = DARK_TEXT, lineGap = 0) {
    ...
    _labelValue(label, text, indent = INDENT) {
        const y = this.getY();
    ...
    _scoreBar(label, score, maxScore = 10, indent = INDENT, barW = mm(70)) {
        if (score === undefined)
            return;
        ...
        if (filled > 0)
            this.doc.rect(barX, trackY, filled, BAR_H).fill();
        // Score label
    ...
    _twoScoreBars(label1, score1, label2, score2) {
    ...
    // Section renderers
    // ---------------------------------------------------------------------------
    _renderPageHeader(php, reportDate) {
        const barH = mm(32);
    ...
    _renderWbs(php) {
        ...
        this.doc.fillColor(DARK_TEXT).text("Over the past month (0 = none of the time/10 = all of the time), on average how often have you been:", L_MARGIN + INDENT, this.getY(), { width: CONTENT_W - INDENT });
    ...
    _renderStrengthsValues(php) {
        if (!php.strengths.length && !php.values.length && !php.vision)
            return;
        ...
        if (php.vision)
            this._tagRow("Vision", [php.vision]);
    }
    _renderNextSteps(php) {
        if (!php.discharge_plan && !php.is_final_session)
            return;
